 ...
 * --- Joseph A. Huwaldt
 */
package java.ui.jahuwaldt.swing;
...
public class AppUtilities {
	...
	**/
	...
	/**
	*  Returns true if this program is running in a MS Windows environment,
	*  false is returned otherwise.
	**/
	...
	**/
	...
	/**
	*  Center the "inside" component inside of the bounding rectangle of
	*  the "outside" component.
	*
	*  @return  A point representing the upper left corner location
	*            required to center the inside component in the
	*            outside one.
	**/
	public static Point centerIt( Component outside, Component inside ) {
		if (outside == null || !outside.isVisible() || !outside.isDisplayable())	return centerIt(inside);
	...
	*
	*  @return  A point representing the upper left corner location
	*            required to center the specified component on the
	*            screen.
	**/
	public static Point centerIt( Component inComponent ) {
		Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
		Dimension componentSize = inComponent.getSize();
	...
	/**
	*  Returns a point that can be used to locate a component in
	*  the "dialog" position (1/3 of the way from the top to the
	*  bottom of the screen).
	**/
	public static Point dialogPosition( Component inComponent ) {
		Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
		Dimension componentSize = inComponent.getSize();
		Point componentLoc = new Point( 0, 0 );
		componentLoc.x = (screenSize.width - componentSize.width) / 2;
	...
	/**
	*  Positions the specified window neatly on the screen such
	*  that document windows are staggered one after the other.
	*  Care is also taken to make sure the window will fit on the screen.
	*
	*  @param  inWindow  The window to be positioned.
	...
	*  Positions the specified window neatly on the screen such
	*  that document windows are staggered one after the other.
	*  This version assumes that windows are a fixed size and can
	*  not be shrunk.
	**/
	...
	/**
	*  Fills in the GridBagConstraints record for a given component
	*  with input items.
	*
	*  @param  gx  Grid (cell) index for this component in the x direction
	*                 (1 == 2nd column).
	*  @param  gy  Grid (cell) index for this component in the y direction
	*                 (3 == 4th row).
	...
	**/
	...
	/**
	*  Try and determine the directory to where the program was installed and
	*  return that as a URL.  Has essentially the same function as
	*  Applet.getDocumentBase(), but works on local file systems
	*  in applications.
	**/
    public static URL getDocumentBase() {
	
		//	Object is to try and figure out where the program was installed.
		...
		if (dir == null || dir.equals("")) {
			...
			} catch (Exception e) { /* Just move on. */ }
	...
	*
	*  @param source  The Component or MenuComponent to find the root
	*                 Component for.
	**/
	public static Component getRootComponent(Object source) {
		...
		if (source instanceof Component)
			...
			if (mParent instanceof Component)
				root = SwingUtilities.getRoot((Component)mParent);
		}
	...
	**/
	...
	/**
	 *  <p>  Build up a JMenu from a description stored in a String
	 *       array.  The description contains the text of each menu
	 *       item, accelerator key, and the name of the method in the specified frame
	 *       that handles the user choosing that menu item.
	 *  </p>
	 *  <p>  This is accomplished by the miracle of reflection.  </p>
	 *
	 ...
	 *  @param  menuDesc  List of String arrays that describes each of the menu items
	 *          that will be built into this menu.  For each String array, item 0 is the menu item
	 *          text string.  If this is <code>null</code>, a menu separator will be placed in
	 *          the menu at this location.  Item 1 is the accelerator key for the menu.
	 *          Item 2 is the name of the method in "parent" that will handle the user
	 *          choosing this menu item. If this is <code>null</code>, the menu item will be shown as disabled.
	...
	*                    action events for this menu's items.
	*  @param  name      Name of the menu to create.
	*  @param  menuDesc  String array that describes each of the menu items
	*          that will be built into this menu.  Item 0 is the menu item
	*          text string.  If this is <code>null</code>, a menu separator will be placed in
	*          the menu at this location.  Item 1 is the accelerator key for the menu.
	*          Item 2 is the name of the method in "parent" that will handle the user
	*          choosing this menu item. If this is <code>null</code>, the menu item will be shown as disabled.
	...
	**/
	...
	/**
	*  <p>  Build up a ButtonGroup containing toggle buttons from a description
	*       stored in a String array.  The description contains, for each button,
	*       the text to appear in the button (<code>null</code> if no text), the name of
	*       the method in the parent object that will be called when a button is
	*       clicked on (if <code>null</code> is passed, the button is disabled), and the tool tip
	*       text to display for the button; (<code>null</code> for no tool tip).
	*  </p>
	*  <p>  This is accomplished by the miracle of reflection.  </p>
	*
	*  @param  parent    The parent object that will contain handle the user clicking
	*                    on one of the buttons in this group.  This object is also
	*                    used to observe the reading in of the button icon images.
	*  @param  defs      String array that describes each of the text string that will
	*          be displayed in the button.  Item 0 is the text to be displayed in each
	*          button.  If this is <code>null</code> no text is displayed.
	*          Item 1 is the name of the method in "parent" that will handle the user
	*          clicking on this button item. If this is <code>null</code>, the menu button will be
	*          shown as disabled.  Item 2 is the tool tip to be shown above this button
	*          item.  If <code>null</code> is passed no tool tip is shown.
	*  @param  imgURLs   An array of URLs for the images to be displayed in the buttons.
	*                    If <code>null</code> is passed in, then there will be no button images.
	*                    If any element is <code>null</code>, that button will have no image.
	...
	*          shown as disabled.  Item 2 is the tool tip to be shown above this button
	*          item.  If <code>null</code> is passed no tool tip is shown.
	*  @param  imgPaths  An array of image file paths for the images to be displayed in the buttons.
	*                    If <code>null</code> is passed in, then there will be no button images.
	*                    If any element is <code>null</code>, that button will have no image.
	*  @return A button group built up from the button description supplied.
	**/
	...
	*  <p>  Build up a ButtonGroup containing toggle buttons from a description
	*       stored in a String array.  The description contains, for each button,
	*       the text to appear in the button (<code>null</code> if no text), the path to an
	*       image file to display in the button (<code>null</code> for no image), the name of
	*       the method in the parent object that will be called when a button is
	*       clicked on (if <code>null</code> is passed, the button is disabled), and the tool tip
	*       text to display for the button (<code>null</code> for no tool tip).
	*  </p>
	...
	*          shown as disabled.  Item 2 is the tool tip to be shown above this button
	*          item.  If <code>null</code> is passed no tool tip is shown.
	*  @param  icons     An array of icon images to be displayed in the buttons.  If this
	*                    array is <code>null</code>, there will be no images displayed in the buttons.
	*                    If one of the elements of the array is <code>null</code>, that button will have
	*                    no image.
	*  @return A button group built up from the button description supplied.
	**/
	...
	/**
	*  <p>  Build up a List of buttons from a description stored in a String
	*       array.  The description contains, for each button, the text to appear in
	*       the button (<code>null</code> if no text), the path to an image file to display
	*       in the button (<code>null</code> for no image), the name of the method in
	*       the parent object that will be called when a button is clicked
	*       on (if <code>null</code> is passed, the button is disabled), and the tool tip
	*       text to display for the button (<code>null</code> for no tool tip).
	*  </p>
	*  <p>  This is accomplished by the miracle of reflection.  </p>
	*
	...
	*                    used to observe the reading in of the button icon images.
	*  @param  defs      String array that describes each of the text string that will
	*          be displayed in the button.  If this is <code>null</code> no text is displayed.
	*          Item 1 is the file path to the image to display in this button.
	*          If <code>null</code> is passed, no image icon is displayed.  Item 2 is the name of the
	*          method in "parent" that will handle the user clicking on this button item.
	*          If this is <code>null</code>, the menu button will be shown as disabled.  Item 3
	*          is the tool tip to be shown above this button item.  If <code>null</code> is passed
	...
	public static Integer userEnteredInteger(Component parent, String msg, String title, int messageType,
		...
		do {
			output = (String)JOptionPane.showInputDialog(parent, msg, title, messageType, null,
											null, new Integer(initialValue));
			if (output != null) {
				try {
					value = Integer.valueOf(output);
					if (value.intValue() < lowerBound || value.intValue() > upperBound)
	...
	*  This is intended to be used as part of processing a user input file name.
	*  <pre>
	...
	*
	*  @param parent    The owner of the dialog (<code>null</code> is fine).
	*  @param theFile  The file to enforce an extension for.  If <code>null</code>, then this dialog does nothing.
	*  @param extension The extension to ensure the file has.  If <code>null</code>, then this method does nothing.
	*  @param existsErrFmtMsg  A MessageFormat compatible "file exists" error message that will have the file name
    *                   substituted into it.  Pass something like, "A file with the named \"{0}\" already exists.
    *                   Do you want to replace it?"
    *  @param dlgTitle  The title of the warning dialog that is shown if the selected file already exists.

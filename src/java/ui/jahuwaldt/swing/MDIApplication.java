...
public class MDIApplication extends Application {
	...
	*/
	private static ArrayList<SoftReference<JMenu>> windowsMenus = new ArrayList<SoftReference<JMenu>>();
	
	/**
	*  A list of objects that want to be notified if the application is going
	*  to quit.
	*/
	...
	*/
	...
	//-------------------------------------------------------------------------
	/**
	*  Constructor a new Application instance with no name.
	*/
	...
	/**
	*  Constructor a new Application instance that has the specified name.
	*
	...
	*/
	...
	//-------------------------------------------------------------------------
	/**
	*  Used to set the resource bundle for this application.  This is provided as
	*  a convenience for storing the resource bundle so that it is accessible
	...
	*/
	...
	/**
	*  Used to set the filename filter for this application.  This is provided as
	*  a convenience for storing the filename filter so that it is accessible
	*  throughout an application.
	*
	...
	*/
	...
	/**
	*  Return a reference to this application's file name filter or null
	*  if a filename filter has not been stored.
	*/
    ...
    /**
     * Sets a flag indicating if the application should quit when the last window
     * is closed (true) or stay open (false; allowing the user to select "New" from
     * the file menu for instance).  The default value is true except on MacOS X where
    ...
    /**
     * Returns a flag indicating if the application should quit when the last window
     * is closed (true) or stay open (false; allowing the user to select "New" from
     * the file menu for instance).  The default value is true except on MacOS X where
     * the default value if false.
     */
    ...
    /**
     * Register the supplied window with the list of windows managed by this MDI
     * Application. Technically, the window should be made visible before calling this
     * method to avoid an inconsistent user interface state (a window listed in the
     * "Windows" menu, but not visible). The window will be removed from the "Windows"
     * menu when it's "dispose" method is called.
     *
     * @param window The window to be added to the list of open windows.
     */
	public void addWindow(Window window) {
        if (openWindows.contains(window)) return;   //  Don't add a window twice.
        
		//  Add the window to the list of open windows.
		openWindows.push(window);
		
		//  Add this window to the Windows menus.
		addNewMenuItem(window);
		
		//  Add a window event listener in order to keep the open windows list and menus up to date.
		window.addWindowListener(new WindowAdapter() {
            @Override
			public void windowClosed(WindowEvent e) {
				Object aWindow = e.getWindow();
				...
				//  On non-Mac systems, quit if there are no open windows.
	...
	*/
	...
	/**
	*  Returns a reference to the top-most window in the list of all open windows registered with
	*  this application.  If there are no open windows, null is returned.
	*/
	...
	*/
	...
	/**
	*  Get a new JMenu instance that can be used as a Windows menu using
	*  the specified title (typically "Windows").  The menu that is returned
	*  is maintained by this class and should not be modified by the user.
	*/
	public JMenu newWindowsMenu(String title) {
		JMenu menu = new JMenu(title);
		
		//  Add the new menu to our list of Windows menus.
		windowsMenus.add(new SoftReference<JMenu>(menu));
	...
	/**
	*  Call this method when a window being shown in a Windows menu has changed
	*  it's title.  This method will update the titles shown in the Windows menu.
	*
	...
	*/
	public void windowTitleChanged(Window window) {
		...
		if (idx >= 0 && windowsMenus.size() > 0 ) {
			...
			for (SoftReference<JMenu> ref : windowsMenus ) {
				JMenu menu = ref.get();
	...
	/**
	*  Method that adds a menu item to the specified menu for each
	*  open window.
	*/
	...
	*/
	private void addNewMenuItem(Window window) {
		if (windowsMenus.size() > 0) {
			...
			String title = getWindowTitle(window);
			for (Iterator<SoftReference<JMenu>> i=windowsMenus.iterator(); i.hasNext();) {
				JMenu menu = i.next().get();
	...
	*/
	...
	/**
	*  Class that provides an action listener for Window menu items.
	*  When this listener is called, it brings it's window to the front.
	*/
	...
	*
	*  @return A reference to the newly created about menu item or null if an item was not created.
	*/
	public AboutJMenuItem createAboutMenuItem() {	return null;	}

	/**
	*  Handle the user requesting a new document window.
	*  If it is inappropriate for the application to create a new document
	*  window, then this method should do nothing.  The default
	*  implementation does nothing and returns null.
	*
	...
	*  @return A reference to the newly created window frame or null if a window was not created.
	*/
	public Frame handleNew( ActionEvent event) { return null; }
	
	/**
	*  Handle the user choosing "Close" from the File menu.  This implementation
	*  dispatches a "Window Closing" event on the top most window.
	*/
	...
	/**
	*  Handle the user choosing "Quit" from the File menu.  This implementation
	*  safely works around a bug in Java on early versions of MacOS X that could
	*  cause a hang when displaying dialogs after choosing quit.  This implementation
	*  dispatches a separate "window closing" event to each window.  This gives each
	*  window an opportunity to cancel the quit process.
	*/

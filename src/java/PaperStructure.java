...
public class PaperStructure extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				/*try {
					//PaperStructure frame = new PaperStructure();
	 ...
	 */
	public PaperStructure(NewPaper a) {
	...
	public PaperStructure(ActionListener actionListener) {
	...
	private void initGUI() {
		...
		pta.addActionListener(new ActionListener() {
		    ...
		    np.g.mcc=n;//setting  total number of mcq quesion in MainFrame
...
"\\end{subqlist}\r\n";*/
  ...
  String essy="\\newpage\r\n"+"\\question{Answer for following question \r\n"+
	...
	String MCQ5=utable+"\\subquestion{"+qu+"\r\n"+dtable+
		...
		for(int i=1;i<=n;i++){
				McqObject ob=new McqObject(i,MCQ);
				ob.subquestion=qu;
				ob.ans1=smp;
				ob.ans2=smp;
				ob.ans3=smp;
		...
		for(int i=1;i<=ns;i++){
			msq+=str;
			StructureQuestion t=new StructureQuestion(i);
		    t.mark=mks1;
			t.title="\\question {"+strtitle+".\r\n";
			t.tmark=Double.parseDouble(mkss);
		    t.utitle=strtitle;
		    StructureObject t1=new StructureObject(1,strsubq);
		    t1.mark=mkss;
		    t.strlist.put(1,t1);
		...
		np.g.str=msq;
		/*McqQuestion testa=new McqQuestion(1);
